...
downloadReport:()=>
{
        ...
        return new Promise(async(resolve,reject)=>
        {
            let Orders=await db.get().collection(collection.ORDERCOLLECTION).find({status:'Delivered'}).toArray()
            resolve(Orders)
...
CouponAd:(Coupon)=>
{
    return new Promise(async(resolve,reject)=>{
      ...
      let response=  await db.get().collection(collection.COUPONCOLLECTION).insertOne(Coupon)
